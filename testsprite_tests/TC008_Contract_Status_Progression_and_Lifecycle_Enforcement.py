...
async def run_test():
    ...
    try:
        ...
        # Select Borrower A to create a new loan request (status REQUESTED).
        ...
        # Fill lender phone number, amount, reason for loan, repayment date, then submit loan request.
        frame = context.pages[-1]
        ...
        # Select a repayment date 5 days from now (2025-10-08) to meet the minimum 48 hours requirement, then submit the loan request.
        frame = context.pages[-1]
        ...
        # Fill the repayment time field (index 8) with a valid time, then submit the loan request.
        frame = context.pages[-1]
        ...
        # Change lender phone number to a valid registered lender phone number and resubmit the loan request.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div[2]/main/div/form/div/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('+919876543211')
        ...
        # Click 'Send Loan Request' button to submit the loan request and create contract with status REQUESTED.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div[2]/main/div/form/div[4]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
