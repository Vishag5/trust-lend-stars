...
async def run_test():
    ...
    try:
        ...
        # Select Lender L1 to start as lender for contract creation and acceptance.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div[2]/div/div[2]/div/button[3]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Create a new contract and verify initial status is REQUESTED.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div[2]/main/div[4]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Fill lender phone number, amount, reason, repayment date, and submit loan request.
        frame = context.pages[-1]
        ...
        # Select a repayment date at least 48 hours from now (current date 2025-10-04) and within 90 days, then submit the loan request.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div[2]/div/div/div/div/table/tbody/tr[2]/td[5]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Click 'Send Loan Request' button to submit the loan request and verify contract status is REQUESTED.
        frame = context.pages[-1]
